...
/**
 * Parse subclass for the "Event" table.
 */
@ParseClassName("Event")
public class Event extends ParseObject {
    public Number getType() { return getNumber("type"); }
    ...
    public String getDescriptionString() {
        ...
        switch (type) {
            ...
            case 2: // created tasklist
                ...
                if (newTaskList == null) return "<Task List Deleted>";
                descriptionString = getUser().getDisplayName() + " created a new task list: " + newTaskList.getListName();
                break;
            case 3: // finished taskList
                TaskList finishedTaskList = (TaskList)getObjects().get(0);
                if (finishedTaskList == null) return "<Task List Deleted>";
                descriptionString = getUser().getDisplayName() + " finished a task list: " + finishedTaskList.getListName();
                break;
            case 4: // created expense
                Expense newExpense = (Expense)getObjects().get(0);
                if (newExpense == null) return "<Expense Deleted>";
                descriptionString = getUser().getDisplayName() + " created a new expense: " + newExpense.getName();
                break;
            case 5: // settled expense
                Expense settledeExpense = (Expense)getObjects().get(0);
                if (settledeExpense == null) return "<Expense Deleted>";
                descriptionString = getUser().getDisplayName() + " created a new expense: " + settledeExpense.getName();
                break;
            default:
